...
class Collection extends EventEmitter {
  ...
  save(docs, cb) {
    ...
    var persis = this[$persis$]
    if (!persis) // not persistent
      return
      ...
      persis.queue(payload.join('\n') + '\n', (err)=> {
          ...
          fs.unlink(toBeDelFilename, (err)=> {
            if (err && 'ENOENT' === err.code) {
              // this file does not exist in normal cases, safely ignore
            } else {
              if (!handleErr(err)) return
            }
            if (!err) {
              logger.warn({
                toBeDelFilename
              }, 'old version of db file is just deleted upon compact, which should had been deleted previously')
            }
            fs.rename(this.filename, toBeDelFilename, (err)=> {
              var renamedTBD = !err
              if (err && 'ENOENT' === err.code) {
                // original db file lost, as new one will be created with in-mem data, just log warning here
                logger.warn({filename: this.filename}, 'db file lost')
              } else {
                if (!handleErr(err)) return
              }
                  ...
                  fs.unlink(toBeDelFilename, (err)=> {
                    if (err) {
                      logger.warn({
                        ...
                        toBeDelFilename
                      }, 'can not delete the old version of db file')
                    }
                    done()
                  })
                } else {
